...
namespace PacmanAI
{
    ...
    public class LucPacScripted : BasePacman
    {
        ...
        protected Direction Ambush(object sender, EventArgs e, GameState gs)
        {
            ...
            if (_ghost != null)
            {
                if (_ghost.Node.ManhattanDistance(gs.Pacman.Node) <
        ...
        protected Direction Wander(object sender, EventArgs e, GameState gs)
        {
            ...
            {
                if (_nearestPill.ManhattanDistance(gs.Pacman.Node) > ENDGAME_DISTANCE_THRESHOLD)
                {
            ...
            if (_ghost != null)
            {
                if (_ghost.Node.ManhattanDistance(gs.Pacman.Node) < FLEE_CHANGE_THRESHOLD)
                {
            ...
            foreach (var item in gs.Map.PillNodes)
            {
                ...
                if (item.ManhattanDistance(gs.Pacman.Node) < AMBUSH_THRESHOLD && item.Type == Node.NodeType.PowerPill)
                {
        ...
        public Direction EndGame(object sender, EventArgs e, GameState gs)
        {
            ...
            if (_nearestPill != null)
            {
                ...
                {
                    if (_nearestPill.Target.ManhattanDistance(gs.Pacman.Node) < ENDGAME_DISTANCE_THRESHOLD)
                    {
