...
namespace PacmanAI
{
    ...
    public class TreeNode
    {
        ...
        public const int MAX_SIMULATIONS = 14;
        public const int SHALLOW_SIMULATIONS = 6;
        #endregion
    ...
    public class LucPac : BasePacman, ICloneable
    {
        ...
        public static GameState SimulateGame(GameState pGameState, BasePacman pController, int pSteps)
        {
            ...
            while (pSteps-- > 0
                   ...
                   && _gameStateCloned.Pacman.Lives >= 0)
            {
        ...
        protected virtual Direction Wander(object sender, EventArgs e, GameState gs)
        {
            ...
            if (m_TreeRoot != null)
            {
                ...
                {
                    if (_nearestPill.ManhattanDistance(gs.Pacman.Node) > ENDGAME_DISTANCE)
                    {
                        return ChangeState(FiniteState.EndGame, true, gs);
                    }
                }
            ...
            {
                if (m_NearestGhostWander.Node.ManhattanDistance(gs.Pacman.Node) < FLEE_CHANGE_THRESHOLD)
                {
            ...
            if (PillNode != null)
            {
                ...
                if (PillNode.ManhattanDistance(gs.Pacman.Node) < AMBUSH_DISTANCE_THRESHOLD)
                {
        ...
        protected Direction Ambush(object sender, EventArgs e, GameState gs)
        {
            ...
            {
                if (item.Node.ManhattanDistance(gs.Pacman.Node) < 
        ...
        public virtual Direction Flee(object sender, EventArgs e, GameState gs)
        {
            ...
            {
                if (m_NearestGhost.Node.ManhattanDistance(gs.Pacman.Node) > FLEE_THRESHOLD)
                {
                    if (_nearestPill.ManhattanDistance(gs.Pacman.Node) > 4)
                    {
                        return ChangeState(FiniteState.EndGame, true, gs);
                    }
                    else
        ...
        public virtual Direction Hunt(object sender, EventArgs e, GameState gs)
        {
            ...
            {
                _nearestGhostDistance = m_FocusGhost.Node.ManhattanDistance(gs.Pacman.Node);
                if (m_FocusGhost.IsEaten || m_FocusGhost.RemainingFlee < 100 * _nearestGhostDistance)
                {
                    m_FocusGhost = null;
                    _nearestGhostDistance = int.MaxValue;
                }
            ...
            if (m_FocusGhost == null)
            {
                foreach (var item in gs.Ghosts)
                {
                    ...
                    var dist = item.Node.ManhattanDistance(gs.Pacman.Node);
                    if (_nearestGhostDistance > dist && 
                        item.Entered &&
                        !item.Chasing )
                    {
                        m_FocusGhost = item;
                        _nearestGhostDistance = item.Node.ManhattanDistance(gs.Pacman.Node);
                    }
                }
            }
            ...
            if (!m_FocusGhost.Fleeing)// || m_FocusGhost.RemainingFlee < 200)
            {
